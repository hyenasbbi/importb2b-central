  ...
  try{
    ...
    if(ids.length){
      const [vr,sr,ir]=await Promise.all([
        ...
        admin.from('importb2b_product_images').select('product_id,image_url,is_primary,sort_order').in('product_id',ids).order('is_primary',{ascending:false}).order('sort_order')
      ]);
    ...
    const im=new Map();for(const x of images)if(!im.has(x.product_id)&&x.image_url)im.set(x.product_id,x.image_url);
    ...
    const safe=(products||[]).map(p=>({id:p.id,name:p.name,category:p.category,description:p.description,image_url:p.primary_image_url||im.get(p.id)||null,variants:vm.get(p.id)||[]})).filter(p=>p.variants.length);
