(function(){
  ...
  async function createBatch(files){
    ...
    const {data,error}=await db.from('importb2b_import_batches').insert({owner_id:u,source:'kyte',status:'uploaded',products_filename:files.products?.name||null,customers_filename:files.customers?.name||null,sales_filename:files.sales?.name||null,metadata:{phase:'phase4',created_from:'import_center'},created_by:u}).select().single();
